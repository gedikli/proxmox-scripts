...
export EPS_APP_CONFIG=$(cat $EPS_BASE_URL/apps/$EPS_APP_NAME/config.sh)
if [ $? -gt 0 ]; then
  log_error "No config found for ${CLR_CYB}$EPS_APP_NAME${CLR}"
fi

EPS_APP_INSTALL=$(cat $EPS_BASE_URL/apps/$EPS_APP_NAME/install.sh)
if [ $? -gt 0 ]; then
  log_error "No install script found for ${CLR_CYB}$EPS_APP_NAME${CLR} in folder ${EPS_BASE_URL}/apps/${EPS_APP_NAME}"
fi
...
export EPS_UTILS_COMMON=$(cat $EPS_BASE_URL/utils/common.sh)
export EPS_UTILS_DISTRO=$(cat $EPS_BASE_URL/utils/${_utilDistro}.sh)
